...
| `decision/runner.py` | 以决策协议运行完整对局，并维护每位 Agent 的行动回合、事件历史、校验重试和默认回退；朝廷玩家的绩效证据与窗口由此接入运行流程。 | 调用 `run_decision_game(engine, controller, artifacts, conversations=..., performance_tracker=...)`；CLI 通过 `DispatchController` 分派至各 Agent。 |
