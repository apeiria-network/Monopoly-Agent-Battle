...
| 字段 | 说明 |
|---|---|
| `provider` | 真实接口使用 `openai_compatible`（OpenAI 兼容网关/直连）或供应商专属接口 `kimi`、`glm`、`gpt`；无网络模拟测试使用 `fake`；固定策略和脚本测试使用 `mock`。 |
| `base_url` | OpenAI 兼容 API 的基地址，程序会请求其 `/chat/completions` 路径。 |
| `api_key_env` | API Key 环境变量名称。 |
| `model` | 供应商使用的模型名称；`openai_compatible`、`kimi`、`glm`、`gpt` 只接受白名单模型（见 7.1）。 |
...
`openai_compatible`、`kimi`、`glm`、`gpt` 四种真实接口 provider 的 `model` 只接受以下模型，配置其他模型名会在加载时直接报错（不区分 `base_url`，对所有地址统一生效）：
...
| `no client factory registered for provider` | 检查 `provider` 是否为 `openai_compatible`、`kimi`、`glm`、`gpt`、`fake` 或 `mock`。 |
