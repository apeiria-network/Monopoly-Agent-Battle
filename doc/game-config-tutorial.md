...
| 字段 | 说明 |
|---|---|
| `provider` | 真实接口使用 `openai_compatible`（OpenAI 兼容网关/直连）或供应商专属接口 `kimi`、`glm`、`gpt`、`qwen`；无网络模拟测试使用 `fake`；固定策略和脚本测试使用 `mock`。 |
| `base_url` | OpenAI 兼容 API 的基地址，程序会请求其 `/chat/completions` 路径。 |
| `api_key_env` | API Key 环境变量名称。 |
| `model` | 供应商使用的模型名称；`openai_compatible`、`kimi`、`glm`、`gpt`、`qwen` 只接受白名单模型（见 7.1）。 |
...
`openai_compatible`、`kimi`、`glm`、`gpt`、`qwen` 五种真实接口 provider 的 `model` 只接受以下模型，配置其他模型名会在加载时直接报错（不区分 `base_url`，对所有地址统一生效）：
...
`kimi-k2.7-code` 已不再支持，配置该模型名会在加载时报错，请改用 `kimi-k2.6`。
...
- **默认关闭**：不写 `thinking` 或写 `false` 即为关闭思考模式；**显式启用**：写 `thinking: true` 才开启。各 provider 实际发送的请求参数不同：
  - `openai_compatible`：开启时发送 `thinking: {"type": "enabled"}`；关闭时不发送该字段。
  - `kimi`（如 kimi-k2.6）：始终显式发送，开启为 `thinking: {"type": "enabled"}`，关闭为 `thinking: {"type": "disabled"}`（该接口默认开启思考，必须显式禁用）。
  - `glm`（如 glm-5.3-flash）：开启时发送 `thinking: {"type": "enabled"}` 并固定附带 `reasoning_effort: "low"`；该接口暂不支持禁用思考，关闭时两个字段都不发送，由服务端默认行为决定。
  - `gpt`（如 gpt-5.6-luna）：开启时发送 `reasoning: {"effort": "low"}`；关闭时不发送该字段。
  - `qwen`（如 qwen3.8-flash）：始终显式发送 `enable_thinking`，开启时为 `true` 并固定附带 `reasoning_effort: "low"`，关闭时为 `false`（该接口各模型服务端默认不一致，必须显式声明）。
...
| `no client factory registered for provider` | 检查 `provider` 是否为 `openai_compatible`、`kimi`、`glm`、`gpt`、`qwen`、`fake` 或 `mock`。 |
| `unsupported model '...' for provider ...` | `model` 只能使用 7.1 节列出的白名单模型；`kimi-k2.7-code` 已移除，请改用 `kimi-k2.6`。 |
