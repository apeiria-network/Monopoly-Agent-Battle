"""Integration coverage for the Tang-ablation advisory-review court."""
...
def _config(output_directory: Path) -> GameConfig:
    return GameConfig(
        game_id="tang-ablation-integration",
        experiment_id="tang-ablation-integration",
        seed=0,
        players=(
            PlayerConfig(
                player_id="tang_abl",
                seat=1,
                controller_type="tang_ablation_court",
...
def _run(
    ...
    conversations = {
        role: AgentConversation(agent_id=f"tang_abl.{role}", window_turns=1)
        for role in ("shangshu", "zhongshu", "menxia", "emperor")
    }
    clients = {
    ...
    controller = DispatchController(
        {"tang_abl": agent, "random": RandomBaselineController(random.Random(1))}
    )
    run_decision_game(
        GameEngine(config),
        controller,
        artifacts,
        conversations={
            "tang_abl": conversations,
            **{f"tang_abl.{r}": c for r, c in conversations.items()},
        },
    )
    return artifacts, captured


def test_tang_ablation_run_audits_roles_and_replays(tmp_path: Path) -> None:
    ...
    tang_decisions = [
        record for record in decisions if record["request"]["player_id"] == "tang_abl"
    ]
    assert tang_decisions
    # 门下省反对也直接进入皇帝终裁：每个决策恰好一轮草拟+一轮审核。
    assert all(
        [call["role"] for call in record["court_trace"]["calls"]]
        == ["shangshu", "zhongshu", "menxia", "emperor"]
        for record in tang_decisions
    )
    assert all(record["court_trace"]["court"] == "tang_ablation" for record in tang_decisions)
    ...
    assert any(
        request.caller_role == "tang_abl.emperor"
        and '"option": "disagree"' in request.messages[-1].content
        for request in captured
    )
    verify_run(artifacts.run_directory)


def test_tang_ablation_internal_retries_and_connection_retry_are_auditable(
    tmp_path: Path,
) -> None:
    ...
    tang_decision = next(
        ...
        if record["request"]["player_id"] == "tang_abl"
    )
    ...
    assert any(
        request.caller_role == "tang_abl.menxia" and "审核结构非法" in request.messages[-1].content
        for request in captured
    )
    verify_run(run_directory)


def test_tang_ablation_shangshu_exhaustion_falls_back_within_runner(tmp_path: Path) -> None:
    def policy(request: LLMRequest) -> str:
        if request.caller_role.endswith(".shangshu"):
    ...
    assert any(
        ...
        for request in captured
        if request.caller_role == "tang_abl.emperor"
    )
    verify_run(run_directory)
