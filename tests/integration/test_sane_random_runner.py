"""Integration tests for sane random game runs."""
...
def _write_sane_config(path: Path, output_directory: Path, game_id: str) -> None:
    path.write_text(
        f"""game_id: {game_id}
experiment_id: sane-random-integration
seed: 17
players:
  - player_id: a
    seat: 1
...
def test_all_sane_random_game_is_auditable_and_never_voluntarily_disposes(
    tmp_path: Path,
) -> None:
    config_path = tmp_path / "sane.yaml"
    output_directory = tmp_path / "runs"
    _write_sane_config(config_path, output_directory, "sane-game")
...
def test_sane_random_run_is_reproducible(tmp_path: Path) -> None:
    first_config = tmp_path / "first.yaml"
    second_config = tmp_path / "second.yaml"
    output_directory = tmp_path / "runs"
    _write_sane_config(first_config, output_directory, "first")
    _write_sane_config(second_config, output_directory, "second")
