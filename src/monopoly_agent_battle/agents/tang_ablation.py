"""Tang-ablation four-role court workflow.

Compared with ``tang_court``, the Menxia review is advisory only: the draft is
never rejected or redrafted. After exactly one draft round and one review, the
emperor reads both and issues the final decision.
"""
...
_ROLE_INSTRUCTIONS = {
    ...
    _ZHONGSHU: _load_prompt("TangAblation/TangAbl_zhongshu.txt"),
    _MENXIA: _load_prompt("TangAblation/TangAbl_menxia.txt"),
    _EMPEROR: _load_prompt("TangAblation/TangAbl_emperor.txt"),
}
_NORMAL_OUTPUT = _load_prompt("normal_output_requirement.txt")
_MENXIA_OUTPUT = _load_prompt("TangAblation/TangAbl_menxia_output_requirement.txt")
_MENXIA_OPTIONS = _load_prompt("TangAblation/TangAbl_menxia_candidates.txt")
...
class TangAblationCourtAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self._trace: list[TangAblationCallTrace] = []
    ...
    def record_final_decision(self, request: DecisionRequest, reply: str) -> None:
        ...
        for index, item in enumerate(self._rounds[-1:], 1):
    ...
    def __call__(self, request: DecisionRequest, feedback: str | None = None) -> str:
        ...
        if self._current_draft is None and not self._rounds:
            self._call_draft(request, 1)
        if not self._rounds:
            review, verdict = self._call_review(request, 1)
    ...
    def _call_review(self, request: DecisionRequest, round_number: int) -> tuple[str, str]:
        ...
        try:
            ...
            raw = json.dumps(
                {
                    "reason": "门下省重连次数耗尽，无法做出有效回复，审核意见仅供参考。",
    ...
    def _messages(
        ...
        pre = _trusted_round_context(self._rounds[-1:]) if role == _EMPEROR else None
        messages, warning = compose_prompt(
...
def _trusted_round_context(rounds: list[_Round]) -> str:
    chunks: list[str] = []
    for item in rounds:
        for role, content_type, raw in (
            (_ZHONGSHU, _DRAFT, item.draft),
...
def _fallback_review() -> str:
    return json.dumps(
        {
            "reason": "门下省多次审核回复非法，无法做出有效回复，审核意见仅供参考。",
            "selected_option": {"option": "disagree"},
        },
        ensure_ascii=False,
    )
